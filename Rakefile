...
def current_gems
  Dir["pkg/usmu-editor-#{Usmu::Editor::VERSION}*.gem"]
end
...
namespace :gem do
  ...
  task :install => ['gem:build'] do
    sh "gem install pkg/usmu-sitemap-#{Usmu::Editor::VERSION}.gem"
  end
  ...
  task :deploy => ['gem:build'] do
    ...
    sh "git tag #{Usmu::Editor::VERSION}" if File.exist? '.git'
  end
end
...
task :changelog, :since_c, :until_c do |t,args|
  ...
  entries = Hash.new
  changelog_content = "\#\# #{Usmu::Editor::VERSION}\n\n"
